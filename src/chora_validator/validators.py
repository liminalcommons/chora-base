...
def validate_manifest(file: os.PathLike[str] | str, policy: dict[str, Any]) -> None:
    ...
    for cap in caps:
        ...
        _require(
            isinstance(behs, list) and behs,
            "capability.behaviors must be non-empty list",
        )
        for b in behs:
            _require(
                "ref" in b and "id" in b and "status" in b,
                "behavior entry must have ref, id, status",
            )
    ...
    _require(
        isinstance(sigs, list) and sigs, "telemetry.signals must be a non-empty list"
    )
    for s in sigs:
        _require(
            "id" in s and "doc" in s and "status" in s,
            "signal must have id, doc, status",
        )
        _require(Path(s["doc"]).exists(), f"signal doc missing: {s['doc']}")


def validate_behaviors(
    path: os.PathLike[str] | str, policy: dict[str, Any] | None = None
) -> None:  # noqa: ARG001
    base = Path(path)
...
def validate_scenarios(manifest_file: os.PathLike[str] | str) -> None:
    ...
    _require(
        ...
        "value_scenarios must be a non-empty list",
    )
    for s in scenarios:
        ...
        _require(
            ...
            f"scenario {s.get('id')} tests must be a non-empty list",
        )
        for t in tests:

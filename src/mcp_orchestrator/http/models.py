...
class Client(BaseModel):
    ...
    client_id: str = Field(
        ..., description="Unique client identifier (e.g., 'claude-desktop')"
    )
...
class DiffConfigResponse(BaseModel):
    ...
    added: list[str] = Field(
        default_factory=list, description="Server IDs added in config2"
    )
    removed: list[str] = Field(
        default_factory=list, description="Server IDs removed from config1"
    )
    modified: list[str] = Field(
        default_factory=list, description="Server IDs with modified parameters"
    )
    diff: str | None = Field(None, description="Human-readable diff summary")
...
class DraftAddRequest(BaseModel):
    ...
    params: dict[str, Any] = Field(
        default_factory=dict, description="Server-specific parameters"
    )
...
class DraftAddResponse(BaseModel):
    ...
    draft: dict[str, Any] | None = Field(
        None, description="Current draft configuration"
    )
...
class DraftRemoveRequest(BaseModel):
    ...
    server_id: str = Field(
        ..., description="Server ID to remove from draft configuration"
    )
...
class PublishConfigResponse(BaseModel):
    ...
    artifact_id: str | None = Field(
        None, description="SHA-256 hash of published artifact"
    )
    message: str | None = Field(None, description="Success or error message")
...
class DeployConfigResponse(BaseModel):
    ...
    message: str | None = Field(None, description="Success or error message")
    deployed: str | None = Field(None, description="Path where config was deployed")
...
class Server(BaseModel):
    ...
    npm_package: str | None = Field(
        None, description="NPM package name (if applicable)"
    )
    command: str | None = Field(None, description="Command to run the server")
...
class InitializeKeysResponse(BaseModel):
    ...
    keys: dict[str, str] | None = Field(None, description="Paths to generated keys")

...
class CLI:
    def __init__(self) -> None:
        parser = argparse.ArgumentParser(
            prog="mcp-orchestrator", description="MCP orchestration CLI"
        )
        ...
        scenarios = sub.add_parser(
            "scenario-validate", help="validate value scenarios via manifest"
        )
    ...
    def run(self, argv=None) -> int:
        ...
        if args.command == "manifest-validate":
            ...
            try:
                from chora_validator.validators import (
                    validate_behaviors,  # type: ignore
                )

...
echo -e "${YELLOW}[1/4] Checking {{ project_slug }} package...${NC}"
if pip show {{ project_slug }} > /dev/null 2>&1; then
    VERSION=$(pip show {{ project_slug }} | grep Version | awk '{print $2}')
    echo -e "  ${GREEN}✓${NC} {{ project_slug }} installed (version $VERSION)"
else
    echo -e "  ${RED}✗${NC} {{ project_slug }} not installed"
    echo "    Run: pip install {{ project_slug }}"
    ERRORS=$((ERRORS + 1))
fi
echo ""

# Check 2: Command available
echo -e "${YELLOW}[2/4] Checking {{ project_slug }} command...${NC}"
if command -v {{ project_slug }} > /dev/null 2>&1; then
    echo -e "  ${GREEN}✓${NC} {{ project_slug }} command available"
else
    echo -e "  ${RED}✗${NC} {{ project_slug }} command not found"
    echo "    Ensure package is installed: pip install {{ project_slug }}"
    ERRORS=$((ERRORS + 1))
fi
echo ""
...
if [ $ERRORS -eq 0 ] && [ $WARNINGS -eq 0 ]; then
    ...
    echo "  3. Check logs: tail -f logs/{{ project_slug }}.log"
    exit 0
    ...
    echo "  1. Install {{ project_slug }}: pip install {{ project_slug }}"

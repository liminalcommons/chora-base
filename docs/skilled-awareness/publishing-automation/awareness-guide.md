# Awareness Guide: Publishing Automation
...
SAP-028 provides secure PyPI publishing with OIDC trusted publishing as default, eliminating long-lived API tokens and enabling PEP 740 build attestations for all chora-base generated projects.

### When to Use This SAP

Use SAP-028 when:
- ✅ Publishing new Python project to PyPI for the first time
- ✅ Migrating existing project from API tokens to OIDC trusted publishing
- ✅ Need PEP 740 attestations for supply chain security / build provenance
- ✅ Eliminating long-lived API tokens (zero secrets management, no rotation burden)
- ✅ Using GitHub Actions for CI/CD workflows
- ✅ Require audit trail and security compliance (OIDC logs, attestation verification)

Don't use SAP-028 for:
- ❌ Non-PyPI registries (Artifactory, Nexus, etc.) - not yet supported
- ❌ Non-GitHub CI platforms (GitLab CI, Bitbucket Pipelines) - planned future support
- ❌ Projects without GitHub repository ownership/admin access (OIDC setup requires admin)
- ❌ Private package registries without OIDC support
- ❌ Legacy CI systems that don't support OIDC workflows
...
```
User request about Publishing Automation
   │
   ├─ [Condition 1]?
...
**Relationship**: [How this SAP depends on SAP-003]

**Agent Workflow**:
1. When using SAP-028, check if SAP-003 is installed
2. If not, recommend installing SAP-003 first
3. [Additional integration steps]


#### SAP-005

**Relationship**: [How this SAP depends on SAP-005]

**Agent Workflow**:
1. When using SAP-028, check if SAP-005 is installed
2. If not, recommend installing SAP-005 first
3. [Additional integration steps]
...
### Explaining Publishing Automation to Users
...
Before completing a task with SAP-028, agents should verify:
...
- **SAP-028 1.0.0** is compatible with:

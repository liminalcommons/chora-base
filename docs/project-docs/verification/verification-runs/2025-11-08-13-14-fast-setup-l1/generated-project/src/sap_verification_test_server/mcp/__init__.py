"""MCP naming conventions for SAP Verification Test Server.
...
NAMESPACE = "sapverify"
"""MCP namespace for SAP Verification Test Server."""
...
def make_tool_name(tool: str) -> str:
    ...
    Returns:
        Namespaced tool name if ENABLE_NAMESPACING=True (e.g., "sapverify:create_task"),
        otherwise returns tool name unchanged.

    Example:
        }}> make_tool_name("create_task")
        "sapverify:create_task"
    """
...
def make_resource_uri(
    ...
    Example:
        ...
        "sapverify://templates/daily-report.md"

        }}> make_resource_uri("docs", "123", {"format": "json"})
        "sapverify://docs/123?format=json"
    """
    uri = "{}://{}/{}".format(NAMESPACE, resource_type, resource_id)

    if query:
        params = "&".join("{}={}".format(k, v) for k, v in query.items())
        uri += "?{}".format(params)
...
def parse_tool_name(full_name: str) -> tuple[str, str]:
    ...
    Args:
        full_name: Full tool name (e.g., "sapverify:create_task")
    ...
    Example:
        }}> parse_tool_name("sapverify:create_task")
        ("sapverify", "create_task")
    """
    if ":" not in full_name:
        raise ValueError(
            "Tool name '{}' missing namespace separator. Expected format: namespace:tool_name".format(
                full_name
            )
        )
...
def parse_resource_uri(uri: str) -> tuple[str, str, str, Optional[dict[str, str}}]:
    """Parse resource URI into components.

    Args:
        uri: Resource URI (e.g., "sapverify://templates/daily-report.md?format=json")
    ...
    Example:
        }}> parse_resource_uri("sapverify://templates/daily-report.md")
        ("sapverify", "templates", "daily-report.md", None)

        }}> parse_resource_uri("sapverify://docs/123?format=json&limit=10")
        ("sapverify", "docs", "123", {"format": "json", "limit": "10"})
    """
    ...
    if "://" not in uri_base:
        raise ValueError(
            "Invalid resource URI: {}. Missing '://' separator. Expected format: namespace://type/id".format(
                uri
            )
        )
    ...
    if "/" not in path:
        raise ValueError(
            "Invalid resource URI: {}. Missing path separator. Expected format: namespace://type/id".format(
                uri
            )
        )
...
def validate_namespace(namespace: str) -> None:
    ...
    Example:
        }}> validate_namespace("sapverify")  # OK
        }}> validate_namespace("My-Project")  # Raises ValueError
    """
    if not NAMESPACE_PATTERN.match(namespace):
        raise ValueError(
            "Invalid namespace: {}. Must be lowercase, 3-20 chars, alphanumeric only. Pattern: [a-z][a-z0-9]{2,19}".format(
                namespace
            )
        )


def validate_tool_name(name: str, expected_namespace: Optional[str] = None) -> None:
    """Validate tool name follows Chora MCP Conventions v1.0.

    Args:
        name: Full tool name (e.g., "sapverify:create_task")
    ...
    Example:
        }}> validate_tool_name("sapverify:create_task")  # OK
        }}> validate_tool_name("CreateTask")  # Raises ValueError (missing namespace)
        }}> validate_tool_name("other:create_task", "sapverify")  # Raises ValueError (wrong namespace)
    """
    ...
    if not TOOL_NAME_PATTERN.match(name):
        raise ValueError(
            "Invalid tool name: {}. Must match pattern: namespace:tool_name (snake_case). Example: {}:create_task".format(
                name, expected_namespace or NAMESPACE
            )
        )
    ...
    if expected_namespace:
        ...
        if actual_ns != expected_namespace:
            raise ValueError(
                "Wrong namespace in tool name. Expected: {}:*, got: {}".format(

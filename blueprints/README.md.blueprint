...
# Memory CLI commands
{{ project_slug }}-memory query --type "event_name"
...
5. **Testing & Quality** → Automated quality gates (≥{{ test_coverage_threshold }}% coverage)
...
| Type check | `mypy src/{{ package_name }}` |
...
Learn how to **build** {{ project_name }}:

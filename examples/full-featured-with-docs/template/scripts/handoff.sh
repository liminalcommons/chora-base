...
# between test-docs-standard and chora-composer codebases.
#
# Usage: ./scripts/handoff.sh [TARGET_INSTANCE]
#   TARGET_INSTANCE: "test-docs-standard" or "chora-composer"
...
if [ -z "$TARGET" ]; then
    ...
    echo "  TARGET_INSTANCE: 'test-docs-standard' or 'chora-composer'"
    exit 1
fi

if [ "$TARGET" != "test-docs-standard" ] && [ "$TARGET" != "chora-composer" ]; then
    echo -e "${RED}Error: TARGET_INSTANCE must be 'test-docs-standard' or 'chora-composer'${NC}"
    exit 1
fi

# Determine current instance
CURRENT_DIR=$(basename "$PWD")
if [ "$CURRENT_DIR" = "test-docs-standard" ]; then
    CURRENT="test-docs-standard"
...
if [ "$TARGET" = "test-docs-standard" ]; then
    echo "     cd /path/to/test-docs-standard"
else

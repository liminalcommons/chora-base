...
echo -e "${YELLOW}[1/4] Checking test-docs-standard package...${NC}"
if pip show test-docs-standard > /dev/null 2>&1; then
    VERSION=$(pip show test-docs-standard | grep Version | awk '{print $2}')
    echo -e "  ${GREEN}✓${NC} test-docs-standard installed (version $VERSION)"
else
    echo -e "  ${RED}✗${NC} test-docs-standard not installed"
    echo "    Run: pip install test-docs-standard"
    ERRORS=$((ERRORS + 1))
fi
echo ""

# Check 2: Command available
echo -e "${YELLOW}[2/4] Checking test-docs-standard command...${NC}"
if command -v test-docs-standard > /dev/null 2>&1; then
    echo -e "  ${GREEN}✓${NC} test-docs-standard command available"
else
    echo -e "  ${RED}✗${NC} test-docs-standard command not found"
    echo "    Ensure package is installed: pip install test-docs-standard"
    ERRORS=$((ERRORS + 1))
fi
echo ""
...
if [ $ERRORS -eq 0 ] && [ $WARNINGS -eq 0 ]; then
    ...
    echo "  3. Check logs: tail -f logs/test-docs-standard.log"
    exit 0
    ...
    echo "  1. Install test-docs-standard: pip install test-docs-standard"

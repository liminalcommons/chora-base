#!/usr/bin/env bash
# diagnose.sh - Automated diagnostics for test-docs-standard
#
...
check_item "Package installed" "pip show test-docs-standard | head -1" ""
...
check_item "src/ directory" "[ -d src/test_docs_standard ] && echo 'Exists' || echo 'Missing'" "Exists"
...
if [ -f logs/test-docs-standard.log ]; then
    LOG_SIZE=$(du -h logs/test-docs-standard.log | cut -f1)
    echo -e "  Log file size: $LOG_SIZE"

    # Check for recent errors
    ERROR_COUNT=$(grep -c "ERROR" logs/test-docs-standard.log 2>/dev/null || echo "0")
    if [ "$ERROR_COUNT" -gt 0 ]; then
        echo -e "  ${YELLOW}⚠${NC} Found $ERROR_COUNT ERROR entries in log"
        echo -e "  ${BLUE}Recent errors:${NC}"
        grep "ERROR" logs/test-docs-standard.log | tail -3 | sed 's/^/    /'
        WARNINGS=$((WARNINGS + 1))
    else
        echo -e "  ${GREEN}✓${NC} No ERROR entries in log"
    fi
else
    echo -e "  ${YELLOW}⚠${NC} No log file found at logs/test-docs-standard.log"
    WARNINGS=$((WARNINGS + 1))
fi
...
if [ "$SAVE_REPORT" = true ]; then
    ...
    echo "  https://github.com/liminalcommons/test-docs-standard/issues/new"
fi

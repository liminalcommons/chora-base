...
echo -e "${YELLOW}[1/4] Checking example-mcp-vision package...${NC}"
if pip show example-mcp-vision > /dev/null 2>&1; then
    VERSION=$(pip show example-mcp-vision | grep Version | awk '{print $2}')
    echo -e "  ${GREEN}✓${NC} example-mcp-vision installed (version $VERSION)"
else
    echo -e "  ${RED}✗${NC} example-mcp-vision not installed"
    echo "    Run: pip install example-mcp-vision"
    ERRORS=$((ERRORS + 1))
fi
echo ""

# Check 2: Command available
echo -e "${YELLOW}[2/4] Checking example-mcp-vision command...${NC}"
if command -v example-mcp-vision > /dev/null 2>&1; then
    echo -e "  ${GREEN}✓${NC} example-mcp-vision command available"
else
    echo -e "  ${RED}✗${NC} example-mcp-vision command not found"
    echo "    Ensure package is installed: pip install example-mcp-vision"
    ERRORS=$((ERRORS + 1))
fi
echo ""
...
if [ $ERRORS -eq 0 ] && [ $WARNINGS -eq 0 ]; then
    ...
    echo "  3. Check logs: tail -f logs/example-mcp-vision.log"
    exit 0
    ...
    echo "  1. Install example-mcp-vision: pip install example-mcp-vision"

...
# between example-mcp-vision and chora-composer codebases.
#
# Usage: ./scripts/handoff.sh [TARGET_INSTANCE]
#   TARGET_INSTANCE: "example-mcp-vision" or "chora-composer"
...
if [ -z "$TARGET" ]; then
    ...
    echo "  TARGET_INSTANCE: 'example-mcp-vision' or 'chora-composer'"
    exit 1
fi

if [ "$TARGET" != "example-mcp-vision" ] && [ "$TARGET" != "chora-composer" ]; then
    echo -e "${RED}Error: TARGET_INSTANCE must be 'example-mcp-vision' or 'chora-composer'${NC}"
    exit 1
fi

# Determine current instance
CURRENT_DIR=$(basename "$PWD")
if [ "$CURRENT_DIR" = "example-mcp-vision" ]; then
    CURRENT="example-mcp-vision"
...
if [ "$TARGET" = "example-mcp-vision" ]; then
    echo "     cd /path/to/example-mcp-vision"
else

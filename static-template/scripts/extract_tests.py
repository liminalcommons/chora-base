...
class TestExtractor:
    ...
    def run(self) -> None:
        ...
        {% raw %}print("Found {} documents with test_extraction: true".format(len(self.docs_with_tests))){% endraw %}
        print()
    ...
    def _extract_python_test(
        ...
        {% raw %}test_name = "test_{}_example_{}".format(safe_title, idx){% endraw %}
...
{}
'''.format(fixture_name, doc_title, self._indent(clean_code, 4)){% endraw %}
...
async def {}():
    ...
    """
{}
'''.format(test_name, doc_title, doc_path, idx, self._indent(code, 4)){% endraw %}
...
def {}():
    ...
    """
{}
'''.format(param_spec, test_name, doc_title, doc_path, idx, self._indent(clean_code, 4)){% endraw %}
...
'''.format(safe_title, idx, doc_title, doc_path, idx, self._indent(clean_code, 4), expected_exit, expected_exit){% endraw %}
...
'''.format(expected_output, expected_output){% endraw %}
...
'''{% raw %}.format(features_text, '\n'.join(imports)){% endraw %}
        ...
        {% raw %}print("✅ Generated {}".format(output_file)){% endraw %}
        {% raw %}print("   Extracted {} test functions".format(len(self.extracted_tests))){% endraw %}
        if self.extracted_fixtures:
            {% raw %}print("   Extracted {} fixtures".format(len(self.extracted_fixtures))){% endraw %}
        if self.requires_async:
...
((TESTS_RUN++))
'''{% raw %}.format(test_name, test_name, test_name){% endraw %}
...
'''
        ...
        {% raw %}print("✅ Generated {}".format(output_file)){% endraw %}
        {% raw %}print("   Extracted {} bash tests".format(len(self.bash_tests))){% endraw %}
        print()

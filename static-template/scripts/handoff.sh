...
# between {{ project_slug }} and chora-composer codebases.
#
# Usage: ./scripts/handoff.sh [TARGET_INSTANCE]
#   TARGET_INSTANCE: "{{ project_slug }}" or "chora-composer"
...
if [ -z "$TARGET" ]; then
    ...
    echo "  TARGET_INSTANCE: '{{ project_slug }}' or 'chora-composer'"
    exit 1
fi

if [ "$TARGET" != "{{ project_slug }}" ] && [ "$TARGET" != "chora-composer" ]; then
    echo -e "${RED}Error: TARGET_INSTANCE must be '{{ project_slug }}' or 'chora-composer'${NC}"
    exit 1
fi
...
if [ "$TARGET" = "{{ project_slug }}" ]; then
    echo "     cd /path/to/{{ project_slug }}"
else

...
if pytest tests/ --cov=src/{{ package_name }} --cov-report=term-missing --cov-fail-under=85 > /dev/null 2>&1; then
    COVERAGE=$(pytest tests/ --cov=src/{{ package_name }} --cov-report=term 2>/dev/null | grep "TOTAL" | awk '{print $4}' || echo "unknown")
    echo -e "  ${GREEN}✓${NC} Tests passed with $COVERAGE coverage"
else
    echo -e "  ${RED}✗${NC} Tests failed or coverage below 85%"
    echo "    Run: pytest tests/ --cov=src/{{ package_name }} --cov-report=term-missing"
    ERRORS=$((ERRORS + 1))
fi
echo ""

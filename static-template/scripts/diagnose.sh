#!/usr/bin/env bash
# diagnose.sh - Automated diagnostics for {{ project_slug }}
#
...
check_item "Package installed" "pip show {{ project_slug }} | head -1" ""
...
check_item "src/ directory" "[ -d src/{{ package_name }} ] && echo 'Exists' || echo 'Missing'" "Exists"
...
if [ -f logs/{{ project_slug }}.log ]; then
    LOG_SIZE=$(du -h logs/{{ project_slug }}.log | cut -f1)
    echo -e "  Log file size: $LOG_SIZE"

    # Check for recent errors
    ERROR_COUNT=$(grep -c "ERROR" logs/{{ project_slug }}.log 2>/dev/null || echo "0")
    if [ "$ERROR_COUNT" -gt 0 ]; then
        echo -e "  ${YELLOW}⚠${NC} Found $ERROR_COUNT ERROR entries in log"
        echo -e "  ${BLUE}Recent errors:${NC}"
        grep "ERROR" logs/{{ project_slug }}.log | tail -3 | sed 's/^/    /'
        WARNINGS=$((WARNINGS + 1))
    else
        echo -e "  ${GREEN}✓${NC} No ERROR entries in log"
    fi
else
    echo -e "  ${YELLOW}⚠${NC} No log file found at logs/{{ project_slug }}.log"
    WARNINGS=$((WARNINGS + 1))
fi
...
if [ "$SAVE_REPORT" = true ]; then
    ...
    echo "  https://github.com/{{ github_username }}/{{ project_slug }}/issues/new"
fi

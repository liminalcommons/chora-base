...
Thank you for your interest in contributing to {{ project_slug }}! This guide will help you get started with contributing code, documentation, bug reports, and feature requests.
...
**Reporting:** If you experience or witness unacceptable behavior, please contact the maintainers at {{ security_email }}.
...
**Without `just`:**
- `just smoke` → `./scripts/smoke-test.sh`
- `just test` → `pytest --cov=src/{{ package_name }} --cov-report=term --cov-fail-under=85`
...
```python
import pytest
from {{ package_name }}.backends.registry import BackendRegistry
...
- **Environment:** Python version, OS, {{ project_slug }} version
...
KeyError: 'CODA_API_KEY'
  File "src/{{ package_name }}/backends/coda_mcp.py", line 42
```
```
...
- Backend implementations in `src/{{ package_name }}/backends/`

**Configuration:**
- Pydantic models in `src/{{ package_name }}/config.py`
...
- **Email** - {{ security_email }} for security issues
...
Thank you for contributing to {{ project_slug }}! 🎉
...
| Lint | `ruff check src/{{ package_name }} tests` |
| Format | `black src/{{ package_name }} tests` |

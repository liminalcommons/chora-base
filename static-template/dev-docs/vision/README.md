# Vision Documents - {{ project_name }}

This directory contains **vision documents** that describe potential future capabilities and evolutionary directions for {{ project_name }}. Vision documents are exploratory, not committed features.

---

## What Are Vision Documents?

Vision documents capture **long-term thinking** about how {{ project_name }} might evolve beyond the current committed roadmap. They serve several purposes:
...
1. Query event log for similar past decisions:
   ```bash
   {{ package_name }}-memory query --type architecture.decision --since 90d
   ```
2. Search knowledge for architectural notes:
   ```bash
   {{ package_name }}-memory knowledge search --tag architecture --tag vision
   ```
